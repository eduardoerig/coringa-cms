...
import { Plus, Edit2, Trash2, Search, Package, X, Save, Upload, Loader2 } from "lucide-react";
...
// Retorna null para URLs externas (não do nosso storage).
function extractStorageFileName(imageUrl: string): string | null {
  try {
    const pathname = new URL(imageUrl).pathname;
    const match = pathname.match(/\/object\/public\/products\/(.+)$/);
...
export default function ProductsAdmin() {
    ...
    if (ok) {
      ...
      if (error) {
        ...
        if (product?.image_url) {
          const fileName = extractStorageFileName(product.image_url);
  ...
  async function handleSaveProduct() {
    ...
    if (selectedFile) {
      // Ao substituir imagem em edição, remove a imagem antiga do storage
      if (modalMode === "edit" && editingProduct?.image_url) {
        const oldFileName = extractStorageFileName(editingProduct.image_url);
    ...
    if (errorResult) {
      ...
      addToast(
        modalMode === "create" ? "Produto criado com sucesso!" : "Produto atualizado!"
      );
      fetchProducts();
    }
  }
  ...
  return (
    <>
      <div className="max-w-6xl mx-auto p-4 pt-24 md:p-10 md:pt-24 space-y-6 overflow-y-auto h-full custom-scrollbar">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-text-500 mt-1">Gerencie os produtos exibidos na landing page.</p>
          </div>
          <button
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-surface-50 rounded-lg flex items-center justify-center p-1 border border-[#1C1C2E]">
                            {product.image_url ? (
                              <Image
                              ...
                              <Package className="w-6 h-6 text-text-300" />
                            )}
                          </div>
                          <div>
                            <p className="font-bold text-text-900 text-sm">{product.title}</p>
                            <p className="text-text-400 text-xs mt-0.5 line-clamp-1 max-w-[200px]">
                              {product.description}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-4 text-sm text-text-500">
                        <span className="bg-text-100/50 px-2.5 py-1 rounded-md">
                          {product.categories?.label || "Sem categoria"}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm text-text-900 font-medium">
                        {product.price
                      ...
                      <td className="px-6 py-4 text-center">
                        {product.is_featured ? (
                          ...
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#1C1C2E] text-[#64748B]">
                            -
                          </span>
                        )}
                      </td>
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="bg-surface-100 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]"
          ...
          >
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <div>
                <label className="block text-sm font-bold text-text-700 mb-1">
                  Nome do Produto
                </label>
                <input
                  type="text"
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {(selectedFile || formData.image_url) && (
                    <div className="relative w-full h-40 bg-surface-50 rounded-xl border border-[#1C1C2E] overflow-hidden group">
                      <Image
                        src={
                          selectedFile
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 p-1.5 bg-surface-100/80 backdrop-blur shadow-sm rounded-full text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  ...
                  <div className="relative">
                    ...
                    <p className="text-[10px] text-text-400 mt-1 px-1">
                      * PNG, JPG ou WebP, máx. 5 MB. Preferência por fundo transparente
                      (PNG), resolução 800×800px.
                    </p>
                  </div>

                  <div className="flex items-center gap-2">
                    <div className="h-px bg-text-100 flex-1"></div>
                    <span className="text-[10px] uppercase font-bold text-text-300">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <label
                  htmlFor="is_featured"
                  className="text-sm font-medium text-text-700 cursor-pointer"
                >
