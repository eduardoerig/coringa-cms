...
export default function CategoriesAdmin() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    if (error) {
      console.error(error);
      addToast(`Erro ao buscar categorias: ${error.message}`, "error");
    }
  ...
  async function handleDelete(id: string, label: string) {
    const ok = await confirm({
      title: `Excluir "${label}"?`,
      message: "Produtos associados ficarão sem categoria. Esta ação não pode ser desfeita.",
  ...
  function openEditModal(category: Category) {
    ...
    setFormData({ label: category.label || "", slug: category.slug || "" });
    setIsModalOpen(true);
  }
  ...
  async function handleSaveCategory() {
    ...
    if (errorResult) {
      addToast(`Erro ao salvar categoria: ${errorResult.message}`, "error");
      console.error(errorResult);
    } else {
      setIsModalOpen(false);
      addToast(modalMode === "create" ? "Categoria criada com sucesso!" : "Categoria atualizada!");
      fetchCategories();
    }
  }
